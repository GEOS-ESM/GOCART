...
!BOP
!
! !MODULE:  Chem_RegistryMod --- Chemistry Registry Class
!
! !INTERFACE:
!
...
!  ------------------------------------------------
   ...
   if ( any(isGOCART) ) then
      ...
      if (.not. ALL(isGOCART(this%i_GOCART:this%j_GOCART))) then
         call die(myname, 'ERROR: non-GOCART entry found in the GOCART block')
      end if
   else
...
!     Determine whether to activate tracer
!     ------------------------------------
      call i90_label ( 'doing_'//trim(name)//':', ier )
      if ( ier .eq. 0 ) then
      ...
      if ( .not. doing_it ) return

!     How many bins do we have?
!     -------------------------
      call i90_label ( 'nbins_'//trim(name)//':', ier )
      if ( ier .eq. 0 ) then
         n_tt = i90_gint ( ier )
      end if
      if ( ier .ne. 0 ) then
         call die ( myname, 'cannot determine NBINS for '//trim(name) )
      end if
  
      if ( n_tt < 1 ) then
         call warn ( myname, 'less than 1 BIN for '//trim(name)// &
                     ' (OK in GEOS-5)' )
      end if

!     Units
!     -----
      call i90_label ( 'units_'//trim(name)//':', ier )
      if ( ier .eq. 0 ) then
         call i90_gtoken ( answer, ier )
         if ( ier .eq. 0 ) then
              units = trim(answer)
      ...
      if ( ier .ne. 0 ) then
         call die ( myname, 'cannot determine UNITS for '//trim(name) )
      end if
...
! This is too restrictive, since it assumes that the first
! specie name is identical to the name of the grid component.
! -----------------------------------------------------------
      if ( .not. doing_it .or. i_tt < 0 ) return

       nbins = j_tt - i_tt + 1
!      if ( nbins == 1 .and. trim(vname) /= 'xx' ) then
!         this%vname(i_tt)  = trim(vname)
!         this%vtitle(i_tt) = trim(vtitle)
!         this%vunits(i_tt)  = trim(vunits)
!      else

! When a species (or bins) name table exists, extract the short names, 
...
!           token='variable_table_'//TRIM(uvname(1:n))//'::'
           ...
           token='variable_table_'//TRIM(uppercase(vname))//'::'
           CALL I90_Label( TRIM(token), ier )

!          Parse variable name table from resource file if it exists
!          ---------------------------------------------------------
RealNames: IF( ier .EQ. 0 ) THEN
		 ...
		 IF( TRIM(token) == '::' ) THEN
		    CALL die( myname, &
                         'Not enough names in chemistry registry file for ' &
                         //TRIM(vname))
		 END IF
	         this%vname(i) = TRIM(token)
                 CALL I90_Gtoken ( token, ier )
	         this%vunits(i) = TRIM(token)
	         CALL I90_Gtoken ( token, ier )
	         this%vtitle(i) = TRIM(token)
	         DO
	            CALL I90_Gtoken ( token, ier )
		    IF ( ier .NE. 0 ) EXIT
		    this%vtitle(i) = TRIM(this%vtitle(i))//" "//TRIM(token)
	         END DO
              END DO

           ELSE

!             Create default names using bin numbers
!             --------------------------------------  
              ibin = 0
              do i = i_tt, j_tt
                 ibin = ibin + 1
                 write(cbin,'(i3.3)') ibin
                 this%vname(i)  = trim(vname)//cbin
                 this%vtitle(i) = trim(vtitle)//' (bin '//cbin//')'
                 this%vunits(i) = trim(vunits)
              end do
