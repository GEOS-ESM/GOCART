...
!      to enable SPSA, set emission_scheme=2024
       real                   :: spsa_ss_gsf     ! emission_scale <- emission_Scale * X (1.0)
       real                   :: spsa_ss_sst0    ! SST term ~ X*SST**0     (1.0)
       real                   :: spsa_ss_sst1    ! SST term ~ X*SST**1     (0.0)
       real                   :: spsa_ss_sst2    ! SST term ~ X*SST**2     (0.0)
       real                   :: spsa_ss_sst3    ! SST term ~ X*SST**3      (0.0)
       real                   :: spsa_ss_w_th    ! u_star <- (u_star - X)  (0.2)
       real                   :: spsa_ss_w_power ! u_star**(2.41*X)        (1.0)
       real                   :: spsa_ss_fscav   ! fscav <- fscav*X        (1.0)
       real                   :: spsa_ss_fwet    ! fwet  <- fwet*X         (1.0)
...
!    call ESMF_ConfigGetAttribute (cfg, self%fscav,      label='fscav:', __RC__)
    ...
    call ESMF_ConfigGetAttribute (cfg, self%spsa_ss_gsf,  label='spsa_ss_gsf:',  default=1.0, __RC__)
    call ESMF_ConfigGetAttribute (cfg, self%spsa_ss_sst0, label='spsa_ss_sst0:', default=1.0, __RC__)
    call ESMF_ConfigGetAttribute (cfg, self%spsa_ss_sst1, label='spsa_ss_sst1:', default=0.0, __RC__)
    call ESMF_ConfigGetAttribute (cfg, self%spsa_ss_sst2, label='spsa_ss_sst2:', default=0.0, __RC__)
    call ESMF_ConfigGetAttribute (cfg, self%spsa_ss_sst3, label='spsa_ss_sst3:', default=0.0, __RC__)
    call ESMF_ConfigGetAttribute (cfg, self%spsa_ss_w_th, label='spsa_ss_w_th:', default=0.2, __RC__)
    call ESMF_ConfigGetAttribute (cfg, self%spsa_ss_w_power, label='spsa_ss_w_power:', default=1.0, __RC__)
    call ESMF_ConfigGetAttribute (cfg, self%spsa_ss_fscav, label='spsa_ss_fscav:', default=1.0, __RC__)
    call ESMF_ConfigGetAttribute (cfg, self%spsa_ss_fwet,  label='spsa_ss_fwet:', default=1.0, __RC__)
...
!      Set SS values to 0 where above klid
    ...
    call ESMF_AttributeSet(field, NAME='ScavengingFractionPerKm', value=self%fscav(1) * self%spsa_ss_fscav, __RC__)
...
!   -----------------------------------------------
    ...
    do n = 1, self%nbins
       ...
       call SeasaltEmission (self%rlow(n), self%rup(n), self%emission_scheme, u10m, &
                             ...
                             self%spsa_ss_w_power, self%spsa_ss_w_th, &
                             __RC__)
...
!      in the surface
       ...
       memissions = self%spsa_ss_gsf  * memissions
