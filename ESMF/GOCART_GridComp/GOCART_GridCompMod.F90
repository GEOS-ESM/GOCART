...
!       VLOCATION  = MAPL_VLocationNone, __RC__)
     ...
     call MAPL_AddImportSpec(GC,                                &
        SHORT_NAME = 'ALBVF',                                   &
! NOTE: generated by SURF Run2, we MAY be able to set MAPL_RestartSkip
...
!   --------------------------------------
    do n = r%i_GOCART, r%j_GOCART 
       ...
       if (state%data_driven) then 
          ...
          if ((r%doing_OH) .and. (n .ge. r%i_OH) .and. (n .le. r%j_OH)) then
             FRIENDLIES = ' '
          else if (trim(r%vname(n)) == 'OX' .and. trim(providerName) == 'GOCART') then
...
!  MANYIN - We now allow additional internal specs, that are added
!           after the n_GOCART specs from the Registry.
...
!  do L = 1, size(InternalSpec)
   do L = 1, chemReg%n_GOCART
      ...
      call MAPL_VarSpecGet(InternalSpec(L), SHORT_NAME=short_name, __RC__)    ! names are  GOCART::<registry_entry>
